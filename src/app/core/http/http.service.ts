...
export class HttpService extends Http {
  ...
  request(request: string|Request, options?: RequestOptionsArgs): Observable<Response> {
      ...
      if ( this.authenticationService.credentials ) {
        console.log('TOken', this.authenticationService.credentials.token)
        authHeader.append('Authorization', 'JWT ' + this.authenticationService.credentials.token);
        // authHeader.append('Authorization', 'JWT eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6InByYWJoYWthcmFuMzA5MEBnbWFpbC5jb20iLCJpYXQiOjE1MzMyODE0MTAsImV4cCI6MTUzMzI4NTAxMH0.owiMqq2T6ResRp75kgFJNxnax5eW_knrN2kR05t5OGs');
      }
  ...
  delete(url: string, body?: any, options?: RequestOptionsArgs): Observable<Response> {
    return this.request(url, extend({}, options, {body: body, method: RequestMethod.Delete }));
  }
